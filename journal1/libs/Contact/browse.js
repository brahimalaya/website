...
var browse={
	...
	applytouchevents:function(dest,touchstart,touchmove,touchend,dontcheck) {
		if (dontcheck) {
			if (!browse._hardware.touchenabled) { // Enable the virtual touch if is not a touch interface.
				...
				if (touchmove) browse.addEventListener(dest,"mousemove",function(e){if(e.stopPropagation) e.stopPropagation(); else e.cancelBubble = true; if (this.ontouchmove) {e.touches=[{identifier:1,clientX:e.pageX,clientY:e.pageY}];e.mouseWrapper=true;this.ontouchmove(e);}});
				if (touchend) browse.addEventListener(dest,"mouseup",function(e){if(e.stopPropagation) e.stopPropagation(); else e.cancelBubble = true;;if(this.ontouchend){e.touches=[];e.mouseWrapper=true;this.ontouchend(e);}});
			}
			...
			if (!browse._hardware.touchenabled) { // Enable the virtual touch if is not a touch interface.
				...
				if (touchmove) browse.addEventListener(dest,"mousemove",function(e){if (!browse.isbrowseridle()) return; if(e.stopPropagation) e.stopPropagation(); else e.cancelBubble = true;; if (this.ontouchmove) {e.touches=[{identifier:1,clientX:e.pageX,clientY:e.pageY}];e.mouseWrapper=true;this.ontouchmove(e);}});
				if (touchend) browse.addEventListener(dest,"mouseup",function(e){if (!browse.isbrowseridle()) return; if(e.stopPropagation) e.stopPropagation(); else e.cancelBubble = true;;if(this.ontouchend){e.touches=[];e.mouseWrapper=true;this.ontouchend(e);}});
			}
	...
	// custom events
	customevents:function(capture) {
		//document.addEventListener("magaka1",function(e){alert("Magaka1");},false);
		//browse.addEventListener("mousemove",function(e){alert("Magaka2");},false);
	...
	layoutbullets:function() {
		if (this._current.magazineversion.gui) {
			if ((browse._current.article.pages.length>1)&&!browse._current.pagedata.scrollpage) {
				//NGE change nov 2012: to allow a fixed position of the pagination bullets
				if (this._current.magazineversion.gui.bulletvpos>0)
					this._bulletbox.style.top=this._current.magazineversion.gui.bulletvpos+"px";
				else
	...
	jumpto2:function(sect,horz,vert) {
		this._current.positions=[];
		this._current.sectionid=sect;
		this._current.positions[horz]=vert;
		this._current.horizontal=horz;
			...
			if (content) {
				...
				if (page.page.substr) // If just a string is specified, the HTML content is set
					...
					if (page.page.template) { // Merge with the inline template and replace placeholders
						...
						// NGE Change nov 2012: to allow having both templates placeholders and article placeholders
						if (this._current.article.placeholders) tx+=this.updatewithplaceholders(tx,this._current.article.placeholders);
						if (page.page.placeholders) tx+=this.updatewithplaceholders(tx,page.page.placeholders);
					}
				}
	...
	// Page resize
	// ---
	updatewithplaceholders:function(tx,ph)
	{
	...
	decideversion:function() {
		var nextversion="";
		for (var a=0;a<browse._magazine.version.length;a++) {
		...
		if (nextversion!=this._current.magazineversionid) {
			...
			//e.initEvent('magaka1', true, true);
			//document.dispatchEvent(e);
			//this._current.horizontal=pos.horz;
			//this.gotohorizontal(this._current.horizontal);
		}
	},
	...
	bindEvent:function (el, eventName, eventHandler) {
		if (el.addEventListener){
			...
			//alert("bindEvent");
			el.attachEvent("onreadystatechange", eventHandler);
		}
	},
	...
	loadjs:function(file) {
		if (!this._loadedfiles[file]) {
			...
			this.bindEvent(js, 'load', this.objloaded);
			...
			case 2: {
				...
				switch (this._settings.tool) {
					...
					default: { // Open magazine
						...
						//alert("browse._magazine.bundle"+browse._magazine.bundle);
	...
	geturldata:function(ud) {
		var request = this.createCORSRequest("get", ud);
		if (request){
			request.onload = function() {
				if (request.readyState == 4) {
					if(request.status == 200)
						this._webcache[ud]=request.responseText;
					else
